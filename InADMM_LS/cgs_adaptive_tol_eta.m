function [x,flag,relres,iter,resvec] = cgs_adaptive_tol_eta(A,b,adaptive_tol,parameters,tol,maxit,M1,M2,x0,varargin)
...
if (normr <= tolb || stag >= maxstagsteps || moresteps)
    ...
    if adaptive_tol==1
       ...
       BB=parameters.BB;
       bb=parameters.bb;
       phi_beta=parameters.phi_beta;
       phi_temp=parameters.phi_temp; 
       lambda2=parameters.lambda2;
       Q=parameters.Q;
        QTE=Q'*x; 
        beta_temp=(phi_temp-QTE)/(lambda2+rhoL);

       
       dd=afun2(beta_temp)-phi_beta;
       if  ~((2/rhoL)*abs(dd'*(ww-beta_temp))+norm(dd)^2 > sigma*norm(AA*beta_temp+BB*alpha-bb-gamma/rhoL)^2)
       break
       end
    end
end                                % for ii = 1 : maxit
