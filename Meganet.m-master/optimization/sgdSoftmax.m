classdef sgdSoftmax < optimizer
    ...
    methods
        ...
        function this = sgdSoftmax(varargin)
            this.stoppingTime = 10; % 10 seconds
            this.maxEpochs = 10;
            this.miniBatch = 16;
            this.atol    = 1e-3;
            this.rtol    = 1e-3;
        ...
        function [xc, xOptLoss, xOptAcc,His] = solve(this,fctn,xc,fval,varargin)
            optValAcc    = 0;
            optValLoss   = Inf;
            for k=1:2:length(varargin)     % overwrites default parameter
...
%                 [Jc,para, dF] = fctn(xc,1:nex); 
                [Jc,para,gradJ] = fctn(xc,1:nex);
%                 fprintf('\nJc = %1.2e\n', Jc);
                if epoch==1; nrm0 = norm(gradJ(:),2); end;
                
                gradRelNorm = norm(gradJ(:),2)/nrm0;
                if doVal
...
%                     his(epoch,5:end) = [gather(objHis(para)), valHis];
                    his(epoch,5:end) = [gather(objHis(para)), valHis, currentRuntime, gradRelNorm];
                    if this.out>0
